module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('svgstack', 'Merge SVG files from a folder.', function() {
        ...
        this.files.forEach(function (file) {
            ...
            file.src.filter(function (filepath) {
                ...
                if (viewBox && !!options.fixedSizeVersion) {
                    ...
                    var $resFixed = cheerio.load('<g><use></use></g>', {lowerCaseAttributeNames: false});
                    var fixedId = graphicId + (options.fixedSizeVersion.suffix || '-fixed-size');
                    var $gFixed = $resFixed('g')
                        .first()
                        .attr('viewBox', [0, 0, fixedWidth, fixedHeight].join(' '))
                        .attr('id', fixedId);
                    Object.keys(options.g).forEach(function (key) {
                        $gFixed.attr(key, options.g[key]);
                    });
                    if (desc) {
                        $gFixed.prepend('<desc>' + desc + '</desc>');
                    }
                    if (title) {
                        $gFixed.prepend('<title>' + title + '</title>');
                    }
